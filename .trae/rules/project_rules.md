...
| 文件特征                      | 自动调用 Skill                   | 用途                  |
| ----------------------------- | -------------------------------- | --------------------- |
| `src/**/formula/**/*.rs`    | `formula-calculation`          | 配方计算逻辑规范      |
| `frontend/src/**/*.tsx`     | `react-typescript-development` | React TypeScript 规范 |
| `src/**/commands.rs`        | `tauri-development`            | Tauri 命令规范        |
| `src/**/*.rs`（非 formula） | `rust-optimization`            | Rust 性能优化         |
| `migrations/**/*.sql`       | `sqlite-optimization`          | 数据库优化            |
...
- **方案 A（推荐）**：使用 Sub-agents - 无物理隔离，简单高效
- **方案 B**：使用 Background Tasks - 耗时任务不阻塞主流程
- **方案 C**：使用 Git Worktree - 仅必要时（实验性开发）
...
- `04-database-standards.md` - 数据库与数据访问规范
